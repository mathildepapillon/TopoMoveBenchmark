"""Structural-lift subjects for datasets WITHOUT explanation ground truth.

Plain cycle lifting (no GT sidecar needed) + StructuralOnes features,
full vocabulary, standard training. Feeds experiments/nogt_usefulness.py.
...
Usage: python train_nogt_subjects.py DATASET
Writes results/nogt_subjects/nogt_{DATASET}.jsonl and checkpoints
under results/nogt_subjects/runs/{DATASET}_s{SEED}/.
"""
...
def main() -> None:
    ...
    out_dir = REPO / "results" / "nogt_subjects"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"nogt_{ds}.jsonl"
    done = {}
    if out_path.exists():
